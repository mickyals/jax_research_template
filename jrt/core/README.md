...
| Name | Description |
|------|-------------|
| `TRANSFORMER_ENCODER` | Sinusoidal/no positional encoding + TransformerBlock stack; `__call__` accepts `return_weights=True` to return `(output, attn_weights)` with weights from every block stacked as `(num_layers, B, H, T, T)` |
