"""
experiments/tc_perceiver_io/train/tune.py

Hyperparameter search for TCPerceiverIO via Optuna.
...
def suggest_fn(trial, base_config: dict) -> dict:
    ...
    cfg["model"]["num_process_layers"] = hp["num_process_layers"]
    cfg["model"]["num_latents"]        = hp["num_latents"]
...
def model_fn(config: dict) -> TCPerceiverIO:
    return TCPerceiverIO(**config["model"])
