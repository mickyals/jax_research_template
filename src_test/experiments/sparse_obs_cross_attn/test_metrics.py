"""
Tests for experiments/sparse_obs_cross_attn/metrics.py.
...
TestBinaryAccuracy    perfect TC detection; perfect no-storm detection; all wrong;
                      output in [0,1]; scalar shape
...
from experiments.sparse_obs_cross_attn.metrics import (
    accuracy,
    binary_accuracy,
    build_metrics_fns,
    cross_entropy,
...
class TestBinaryAccuracy:
    ...
    def test_perfect_tc_detection(self):
        labels = jnp.ones(B, dtype=jnp.int32) * 5   # all TC (class 5)
        logits = _confident_logits(5)                # correctly predict TC
        assert float(binary_accuracy(logits, labels)) == pytest.approx(1.0)

    def test_perfect_no_storm_detection(self):
        labels = jnp.zeros(B, dtype=jnp.int32)      # all background
        logits = _confident_logits(0)                # correctly predict class 0
        assert float(binary_accuracy(logits, labels)) == pytest.approx(1.0)

    def test_all_wrong_binary_gives_zero(self):
        labels = jnp.zeros(B, dtype=jnp.int32)      # all background
        logits = _confident_logits(5)                # predict TC for everything
