...
├── ibtracs.py          IBTrACSDataset — season splits, multi-storm filtering,
│                           SSHS label mapping
...
```
# .gitignore
src/experiments/sparse_obs_cross_attn/configs/*_local.yaml
```
...
import sys, os
sys.path.insert(0, 'src')        # resolves `from experiments.* import ...`
...
with open('src/experiments/sparse_obs_cross_attn/configs/tc_classifier.yaml') as f:
...
from experiments.sparse_obs_cross_attn.evaluate import (
    collect_predictions, confusion_matrix,
    plot_confusion_matrix, plot_class_metrics, per_class_metrics, CLASS_NAMES,
)
