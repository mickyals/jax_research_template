...
class TestDataModuleInterface:
    ...
    def test_manifest_reports_resolved_split(self, dm):
        manifest = dm.manifest()
        assert manifest["split"]["column"] == "SEASON"
        assert manifest["train"] == {"values": [2010], "n_rows": 200}
        assert manifest["val"]   == {"values": [2021], "n_rows": 50}
        assert manifest["test"]  == {"values": [2023], "n_rows": 50}
